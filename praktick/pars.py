...
for i in range(count_iter):
    ...
    if response.status_code == 200:
        ...
        for url_of_news in news_links:
            url_of_news = urljoin("https://lenta.ru/", url_of_news)
            page_of_news = requests.get(url_of_news)
            soup_news = BeautifulSoup(page_of_news.content, 'html.parser')

            r = requests.get(url_of_news)
            ...
            try:
                if soup_news is not None :
                    ...
                    if len(news_contents) > 0:
                        ...
                        file_name = f"{re.sub(r'/', '_', r_date)}_Lenta.ru_{r_teg}_{number_of_news}.txt"
                ...
                print(f"Exception occurred while processing {url_of_news}: {e}")

            visited_links.add(url_of_news)
        count_for_pars += 1
